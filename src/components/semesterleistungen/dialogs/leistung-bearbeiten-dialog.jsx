...
import { ChevronDownIcon, CalendarIcon, CheckIcon, TrashIcon } from '../../ui/icon';
import { useSemesterLeistungen } from '../../../contexts/semester-leistungen-context';
import { useLabels } from '../../../hooks/use-labels';

/**
 * LeistungBearbeitenDialog - Dialog for editing an existing semester performance
 *
 * Figma: Node-ID 2122:7308
 * Layout: Two columns (identical to NeueLeistungDialog)
 */
const LeistungBearbeitenDialog = ({ open, onOpenChange, leistung, onSave, onDelete }) => {
  ...
  // Load leistung data when dialog opens
  useEffect(() => {
    if (open && leistung) {
      setFormData({
        ...
        ects: leistung.ects?.toString() || '',
        note: leistung.note?.toString() || '',
        status: leistung.status || 'ausstehend',
        inKalender: leistung.inKalender || false,
        // T29: Load reminder fields
        erinnerungEnabled: leistung.erinnerungEnabled || false,
        erinnerungDatum: leistung.erinnerungDatum || '',
      });
      setIsRechtsgebietOpen(false);
    }
  }, [open, leistung]);

  // T29: Calculate default reminder date (3 weeks before exam) when enabling reminder
  useEffect(() => {
      ...
      <DialogContent className="relative max-w-2xl">
        ...
        <DialogFooter className="flex items-center justify-between">
          ...
          <Button
            variant="ghost"
            onClick={onDelete}
            className="flex items-center gap-1.5 text-neutral-600 hover:text-red-600 hover:bg-red-50"
          >
            Leistung löschen
            <TrashIcon size={14} />
          </Button>
...
export default LeistungBearbeitenDialog;
